 ...
 */
// Deno copy of src/integrations/shopify/mapper.ts — keep the two in sync.
// It takes a minimal slice of the dataset so the Edge Function only has to
// fetch four tables.
type Row = Record<string, any>;
interface MapperDb { cities: Row[]; variants: Row[]; products: Row[]; campaigns: Row[] }
type ParsedExternalOrder = {
  externalId: string; idempotencyKey: string; warnings: string[]; input: Row;
};
...
export function parseShopifyOrder(payload: unknown, db: MapperDb): ParsedExternalOrder | null {
